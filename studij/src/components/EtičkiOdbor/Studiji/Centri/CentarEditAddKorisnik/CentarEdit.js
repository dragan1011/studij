...
const ModalOverlay = (props) => {
  ...
  const statusKorisnika = [{"id":1,"naziv":"Aktivan"},
  ...
  return (
    <div>
      ...
      <div onClick={() => {props.closeModal(false)}} className={classes.backdrop} />
        ...
        <TabButton name={"Izmjena"} activ={active} select={setSelectedHandler}>Korisnici</TabButton>
       <TabButton name={"Recept"} activ={active}  select={setSelectedHandler}>{props.title}</TabButton>
        </header>
   ...
   <label className={classes.label}>Ime i prezime</label>
    <input onChange={e => setKorisnik(e.target.value)} className={classes.input} type="text" />
   </div>
   <div className={classes.smallWrapper}>
...
<input
   ...
   <label className={classes.label}>Naziv centra</label>
    <input ref={nazivRef} onChange={e => setNaziv(e.target.value)} defaultValue={props.data.naziv} className={`${classes.input} ${nazivIsValid&&(naziv.trim()===null || naziv.trim() === '')  ? classes.border : ''}`} type="text" />
   </div>
   <div className={classes.smallWrapper}>
   <label className={classes.label}>Šifra centra</label>
    <input ref={sifraRef} onChange={e => setSifra(e.target.value)} defaultValue={props.data.sifra} className={`${classes.input} ${sifraIsValid&&(sifra.trim()===null || sifra.trim() === '')  ? classes.border : ''}`} type="text" />
   </div>
   <footer className={classes.actions}>

...
export default function DijetaTabela(props) {
  ...
  return (
    <div className={classes.dijeteWrapper}>
      ...
      {Array.from(props.data).sort((a,b) => +a.sifra.slice(3,6) > +b.sifra.slice(3,6) ? 1 : -1).map(item =>
            
    
      Number(props.studijId) === Number(item.id_studija) ?
         <CentriRed centar={props.centar} centarId={props.centarId} key={Math.random()} {...item} open={openModalHandler}></CentriRed>
         : ''

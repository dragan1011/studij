...
import classes from './AddRezimCuvanjaModal.module.css';
   ...
   const addNewData = async (e) => {
    ...
    if (opis === ''|| opis === null || opis.length>0&&opis.length<3) {
     ...
     setTimeout( async()=> {
      ...
      Axios.post('http://localhost:3001/rezimCuvanjaDodaj', {
        naziv: naziv,
        opis: opis
      }).then((response)=> {
        props.refresh();
...
{/*       {lokacijaIsValid&&lokacija==='Izaberite grupu' ? <label className={classes.labelUpozorenja}>Morate izabrati grupu namirnica!</label>:""}
  ...
  */}  </div>
   ...
   <footer className={classes.actions}>
          <button type='submit' className={classes.button}>Dodaj status</button>
